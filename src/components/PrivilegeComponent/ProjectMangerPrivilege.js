...
export default class ProjectMangerPrivilege extends Component {
    state = {
        PmPrivilages: ''
    }
    ...
    componentDidMount() {
        axios.get(API_BASE_URL + "/getAlPMPrivilegeInfo", { headers: { Authorization: 'Bearer ' + localStorage.getItem(ACCESS_TOKEN) } })
            .then(res => {
                res.data.map((post, index) => {

                })
                this.setState({ PmPrivilages: res.data })
            })
    }
    ...
    submit = () => {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            axios
                .put(API_BASE_URL + "/updatePMPrivilege/" + idlist[i], list[i], { headers: { Authorization: 'Bearer ' + localStorage.getItem(ACCESS_TOKEN) } })
                .then(res => {
    ...
    render() {
        ...
        {
            path: 'second',
            breadcrumbName: 'Project Manger Privilege',
        },
        ];
        return (
            <React.Fragment>
                <PageHeader title="Project Manger Privilege" breadcrumb={{ routes }} />
                <div
                    style={{
